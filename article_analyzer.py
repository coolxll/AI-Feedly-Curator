...
配置：
    修改 config.py 中的 PROJ_CONFIG 来调整默认设置
...
# 导入模块
from config import PROJ_CONFIG, setup_logging
from feedly_client import feedly_fetch_unread, feedly_mark_read
from article_fetcher import fetch_article_content
from llm_analyzer import analyze_article_with_llm, generate_overall_summary
...
def main():
    ...
    parser.add_argument("--refresh", action="store_true", default=PROJ_CONFIG["refresh"], 
                        help=f"Refresh unread_news.json from Feedly before processing (default: {PROJ_CONFIG['refresh']})")
