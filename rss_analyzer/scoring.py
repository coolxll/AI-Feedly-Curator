...
def build_scoring_prompt(title: str, summary: str, content: str) -> str:
    ...
    Returns:
        评分提示词
    """
    return f"""你是一名专业内容编辑，请根据下面的标准判断给定文章是否值得普通读者花时间完整阅读。

评分维度：

1. 相关性（1–5 分）：文章内容是否紧扣给定主题或读者需求。

2. 信息量与准确性（1–5 分）：是否提供新的、有用的信息，是否基本可靠，无明显事实错误。

3. 深度与观点（1–5 分）：是否有有价值的分析和观点，而不是只堆砌表面结论或常识。

4. 可读性（1–5 分）：结构是否清晰、语言是否流畅、逻辑是否连贯。

5. 重复度/水分（1–5 分）：分数越高表示"水分越少"，不是简单复制常识或广告文案。
